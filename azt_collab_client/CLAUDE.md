...
| LAN peer identity (per-device ed25519 keypair + X.509 cert + sha256 fingerprint) | `lan_peer_id` (`GET /v1/lan/peer_id`) | Pairing QR can't be generated; peers can't authenticate against this daemon's LAN listener. Created lazily on first read; persisted as `$AZT_HOME/peer_id` + `$AZT_HOME/peer.crt`. Since 0.45.0. |
