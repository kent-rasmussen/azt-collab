...
| Share-bundle staging (per-call cache) | `prepare_share_bundle` (`POST /v1/diagnostics/prepare_share_bundle`) writes a single gzipped-tar containing the snapshot + per-day daemon logs as `$AZT_HOME/.shares/<token>/azt_diagnostics_<stamp>.tar.gz`; `AZTCollabProvider.openFile` serves the URI (`content://org.atoznback.aztcollab/_shares/<token>/<archive>`). Returns one item with one `uri_path` (MIME `application/gzip`; `share_diagnostics_action` sets the intent type to match). Stale tokens >1h pruned on every call. | Without daemon-staged URIs from our own authority, share intents get silently rejected by Signal. Without bundling into one ACTION_SEND-shaped file, Signal's SEND_MULTIPLE runtime filter (image/video only) drops the URIs. `.tar.gz` not `.zip` because a field email server (Dome) strips `.zip` — gzip magic bytes dodge both extension and content-sniffing filters. The container FORMAT (tar.gz + name + MIME) is the shared `azt_collab_client.diagnostics` helper (`build_diagnostics_targz` / `diagnostics_archive_name` / `DIAGNOSTICS_MIME`) so daemon and peers can't drift; only collection/staging is daemon-specific. Any fault loses the diagnostic. Since 0.52.13 (URI staging) / 0.52.19 (single-file + ACTION_SEND) / 0.52.23 (`.tar.gz`) / 0.52.27 (shared format helper). |
