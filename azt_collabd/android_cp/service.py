...
def _resolve_path(rel, mode):
    ...
    # Diagnostic share routing (since 0.52.13): ``_shares/<token>/<filename>``
    # maps to ``$AZT_HOME/.shares/<token>/<filename>``. Used by the
    # picker / settings ``Share diagnostics`` button to ship URIs
    # under THIS APK's authority instead of MediaStore. Signal
    # refuses MediaStore Downloads URIs (security policy) but
    # accepts URIs from the sender's own ContentProvider authority.
    # Files are written by the daemon via
    # ``_h_prepare_share_bundle`` and read-only here.
    if len(parts) == 3 and parts[0] == '_shares':
        return _resolve_share_path(parts[1], parts[2], mode)
...
def _resolve_share_path(token, filename, mode):
    """Map ``_shares/<token>/<filename>`` to
    ``$AZT_HOME/.shares/<token>/<filename>``. Read-only — peers
    consuming the share intent open the file for reading via
    ``ContentResolver.openInputStream``; the daemon is the only
    writer (through ``_h_prepare_share_bundle``).

    Used by the picker / settings ``Share diagnostics`` button
    (since 0.52.13) to ship URIs under THIS APK's authority
    instead of MediaStore. Signal refuses MediaStore Downloads
    URIs (its security policy whitelist) but accepts URIs from
    the sender's own ContentProvider authority.

    ``token`` is generated by the daemon via
    ``secrets.token_hex(16)`` so it passes ``_is_safe_pending_token``
    (the regex predates this path and is reused).

    ``filename`` is sanitised by rejecting anything outside
    ``[A-Za-z0-9._-]`` — the daemon controls what filenames it
    puts in the bundle (snapshot + per-day daemon logs), and
    those are all safe shapes, but the structural check belts-
    and-braces against a daemon bug that might let a hostile
    filename through.

    Returns ``None`` on token / filename validation failure or
    when the file doesn't exist."""
    if not _is_safe_pending_token(token):
        return None
    import re
    if not isinstance(filename, str) or not re.match(
            r'^[A-Za-z0-9._-]{1,128}$', filename):
        return None
    if mode and ('w' in mode or 'a' in mode):
        # Peers can't write to share files — daemon owns them.
        return None
    base = os.path.realpath(os.path.join(azt_home(), '.shares'))
    target = os.path.realpath(os.path.join(base, token, filename))
    try:
        if os.path.commonpath([base, target]) != base:
            return None
    except ValueError:
