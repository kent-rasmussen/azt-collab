...
10. **LAN sync is opportunistic peer-to-peer; github stays authoritative (0.45.0).** Each daemon binds a TLS HTTPS listener on `0.0.0.0:0` when `lan.allow_sync` is on (hot-applied — toggle flip doesn't restart the daemon). Per-device ed25519 keypair + self-signed X.509 cert in `$AZT_HOME/peer_id` / `peer.crt`; peers pin each other by SHA-256 fingerprint via `peers.json`. TLS client-cert validation is **deliberately CERT_NONE** — stdlib `ssl` has no "request cert + skip CA chain" mode, so identity is asserted in the body (signed claim against the recorded fp) and bound to user gesture (QR display) before any auto-share. **LAN delivery never clears `pending_push`** — only a successful github push does (per the "github convergence" property in the parked spec). The scheduler's `_run_commit` fires `lan_push.fan_out` after each `COMMITTED_LOCAL` and the watcher's drain loop fans out periodically. Discovery: desktop uses python-zeroconf; Android uses `NsdManager` with `static_endpoints` as the hotspot-host fallback. The fan-out and the merge path go through the same `repo._merge_diverged` as github sync; the LAN merge holds `project_lock` for the fetch + merge + push (see `lan_push._merge_then_push`).
