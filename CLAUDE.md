...
13. **No duplicate same-lang forms in SINGLE-FORM fields — but `<gloss>` is legitimately multi-per-lang (0.54.0; gloss carve-out 0.54.20).** `_normalize_entry` runs on every entry of every `three_way_merge` output (all call sites). For **single-form-per-lang** contexts (lexical-unit `<form>`, `<field type="…verification…">` forms): identical same-lang duplicates collapse to the document-first node; verification-field duplicates union their python-list code content with **byte-identical semantics to azt's `Field.consolidate_forms_by_lang`** (first-seen order; a check verified to different values on the two sides is dropped — it must re-verify) — if you change one layer's union semantics you must change both; any other same-lang multiplicity survives only as an annotated `azt-lift-conflict` pair. **`<gloss>` is the exception and MUST NOT be treated as single-form-per-lang:** a sense legitimately carries multiple same-`@lang` glosses (distinct senses/synonyms — the bare CAWL template ships e.g. two `<gloss lang="es">`: `abdomen`, `barriga`). For glosses, only byte-identical duplicates collapse; **distinct same-lang glosses are real data, never a conflict, never annotated**, and `_normalize_entry` strips any `azt-lift-conflict` annotation a pre-0.54.20 merge stamped on them. **This "same-lang glosses never conflict" rule is WORKFLOW-GATED, not permanent:** it holds only while glosses are read-only (no edit UI), so all same-lang multiplicity is distinct senses. **When gloss editing ships, revisit this** — two different same-lang glosses become ambiguous (new synonym vs. one gloss edited to divergent values = a real conflict), and the merge will need per-gloss identity (base-match / id, like `_pair_same_key`) to distinguish them. Field repro 2026-07-22 (CABTAL): the single-form rule was mis-applied to glosses, falsely annotating every multi-sense entry — 1284 false markers in the workshop nml, over fully-intact data. `definition`/`citation` forms are also true multitext (verify before extending the single-form rule to them). Same-key children pair by content (`_pair_same_key`), not position, and one-sided children unchanged since base are honored as deletes — pre-0.54.0 positional pairing + base-blind keeps multiplied one duplicate per merge (field repro 2026-07-10: 'wife' entry, 29 same-lang forms in one verification field, one computer).
